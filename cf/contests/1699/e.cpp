...
void test_case() {
    ...
    vector<int>mx(m + 3, 0), a(n);
    int min_a = 1 << 30;
    for(int i = 0; i < n; i++) {
        scanf("%d", &a[i]);
        min_a = min(min_a, a[i]);
        mx[1] = max(mx[1], second(intervals[a[i]][0]));
        for(int j = 1; j < (int)intervals[a[i]].size(); j++) {
            int k = first(intervals[a[i]][j - 1]);
            mx[k + 1] = max(mx[k + 1], second(intervals[a[i]][j]));
        }
    }
